...
export class TopicComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.createEditForm = this.formBuilder.group({
      ...
      details_bn: [null, [Validators.required]],
  ...
  getList() {
    ...
    this._service.get("topics").subscribe(
      (res) => {
  ...
  getTopicMaxSequence() {
    this.loadingIndicator = true;
    this._service.get("topic-max-sequence").subscribe(
      (res) => {
  ...
  openModal(template: TemplateRef<any>) {
    ...
    this.imageUrl=null;
    this.modalTitle='Create new Topic';
  ...
  openUpdateModal(template: TemplateRef<any>, data) {
    this.createEditForm.reset();
    this.modalTitle='Update Topic Details';
    this.btnSaveText='Update Topic';
    if(data.class_id){
  ...
  onFormSubmit() {
    ...
    if(!this.createEditForm.value.id){ // create new Topic ----------------
      ...
      formData.append('name_bn', this.createEditForm.value.name_bn)
      formData.append('name_en', this.createEditForm.value.name_en)
      ...
      this._service.post('topics', formData).subscribe(
        res => {
          this.blockUI.stop();
      ...
      this._service.put('topics/'+this.createEditForm.value.id, obj).subscribe(
        res => {
          this.blockUI.stop();
            this.toastr.success(res.messages, 'Success!', { timeOut: 2000 });
            this.submitted = false;
  ...
  submitDeleteForm(school) {
    ...
    this.confirmService.confirm('Are you sure?', 'Do you want to delete the this Topic?')
    .subscribe(
        result => {
            if (result) {
                // delete school ----------------
              this.blockUI.start('Deleting...');
              this._service.delete('topics/'+school.id).subscribe(
                res => {
                  this.blockUI.stop();
                    this.toastr.success(res.messages, 'Success!', { timeOut: 2000 });
                    this.submitted = false;
  ...
  onPhotoSubmit() {
    ...
    this._service.post('topic-icon-change', formData).subscribe(
      res => {
        this.blockUI.stop();
