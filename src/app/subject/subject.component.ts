...
export class SubjectComponent implements OnInit {
  ...
  getList() {
    ...
    this._service.get("subjects").subscribe(
      (res) => {
        console.log(res)
  ...
  getSubjectMaxSequence() {
    this.loadingIndicator = true;
    this._service.get("subject-max-sequence").subscribe(
      (res) => {
  ...
  openModal(template: TemplateRef<any>) {
    ...
    this.imageUrl=null;
    this.userId=null;
    this.modalTitle='Create new Subject';
  ...
  onFormSubmit() {
    ...
    if(!this.createEditForm.value.id){ // create new class ----------------
      ...
      this._service.post('subjects', formData).subscribe(
        res => {
          this.blockUI.stop();
      ...
      this._service.put('subjects/'+this.createEditForm.value.id, obj).subscribe(
        res => {
          this.blockUI.stop();
            this.toastr.success(res.messages, 'Success!', { timeOut: 2000 });
  ...
  submitDeleteForm(data) {
    this.confirmService.confirm('Are you sure?', 'Do you want to delete the this Subject ?')
    .subscribe(
        result => {
            if (result) {
                // delete school ----------------
              this.blockUI.start('Deleting...');
              this._service.delete('subjects/'+data.id).subscribe(
                res => {
                  this.blockUI.stop();
                    this.toastr.success(res.messages, 'Success!', { timeOut: 2000 });
                    this.submitted = false;
